...
SELECT FUNCION_SUMAR(12,12) FROM DUAL;

--16 Crear una función que visualice una cadena al revés.

CREATE OR REPLACE FUNCTION AL_REVES(COSA VARCHAR2)
RETURN VARCHAR
IS
	ROBLE VARCHAR2(65);
BEGIN
...
select DAR_FECHA('24-06-2016') "Fecha con función" FROM DUAL;
...
CREATE OR REPLACE FUNCTION DAME_IVA(PRECIO NUMBER, tipo number default 0.16)
RETURN NUMBER
IS
	FINAL NUMBER;
BEGIN
...
SELECT DAME_IVA(500,0.2) FROM DUAL;

SELECT DAME_IVA(500) FROM DUAL;

/* 19- Utilizar la función anterior para obtener el salario de
los empleados con el iva de la función. */

SELECT EMP_NO, SALARIO, DAME_IVA(SALARIO) "IVA" FROM EMPLE;