...
(select oficio from emple where apellido like 'JIMENEZ')
or SALARIO >=
(select salario from emple where apellido like 'FERNANDEZ');
...
--like 'Medicina%'; en caso de que se pida una 

-- no devuelve resultados a pesar de que debería hacerlo (POR FALLO al escribirlo con minúsculas)
...
select distinct nombre nombre from asignaturas, notas
